...
opt,sol,data = tssos(f, x, newton=true, reducebasis=true, TS="block", solution=true, Gram=true, QUIET=true)
@test opt ≈ 0.4752748 atol = 1e-6
opt,sol,data = tssos(f, x, newton=false, reducebasis=false, TS="block", solution=true, Gram=true, QUIET=true)
@test opt ≈ 0.4752748 atol = 1e-6
opt,sol,data = tssos(data, TS="MD", solution=true, Gram=true, QUIET=true)
@test opt ≈ 0.4752748 atol = 1e-6
...
pop = [f, g, h]
opt,sol,data = tssos(pop, x, 2, numeq=1, GroebnerBasis=false, TS="block", Gram=true, solution=true, QUIET=true, normality=true)
@test opt ≈ 0.9555474 atol = 1e-6
opt,sol,data = tssos(pop, x, 2, numeq=1, GroebnerBasis=true, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.9555474 atol = 1e-6
opt,sol,data = tssos(data, TS="block", solution=true, Gram=true, QUIET=true)
@test opt ≈ 0.9555474 atol = 1e-6
opt,sol,data = tssos(pop, x, 2, numeq=1, GroebnerBasis=true, TS="MD", Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.9555474 atol = 1e-6
opt,sol,data = tssos(pop, x, 2, numeq=1, GroebnerBasis=false, TS=false, Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.9555474 atol = 1e-6
opt,sol,data = tssos(pop, x, 2, numeq=1, GroebnerBasis=false, TS="signsymmetry", Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.9555474 atol = 1e-6

@polyvar x[1:2]
pop = [-x[1]-x[2], 2x[1]^4-8x[1]^3+8x[1]^2+2-x[2], 4x[1]^4-32x[1]^3+88x[1]^2-96x[1]+36-x[2], (3-x[1])*x[1], (4-x[2])*x[2]]
opt,sol,data = tssos(pop, x, 3, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ -5.508013 atol = 1e-6

@polyvar x[1:3]
pop = [x[1]^2*x[2]^2*(x[1]^2+x[2]^2-3x[3]^2)+x[3]^6, 1-x[1]^2-x[2]^2-x[3]^2]
opt,sol,data = tssos(pop, x, 3, TS=false, Gram=true, solution=true, QUIET=true)
...
opt,sol,data = cs_tssos(pop, x, 2, numeq=3, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ -0.4142135 atol = 1e-6
opt,sol,data = cs_tssos(data, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ -0.4142135 atol = 1e-6
...
(10-x[6])*x[6], x[1], x[2]]
d = 2
opt,sol,data = tssos(pop, x, d, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ -310 atol = 1e-3
opt,sol,data = cs_tssos(pop, x, d, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ -310 atol = 1e-3
...
d = 2
opt,sol,data = cs_tssos(pop, x, d, numeq=1, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.71742533 atol = 1e-6
opt,sol,data = cs_tssos(data, TS="MD", solution=true, Gram=true, QUIET=true)
@test opt ≈ 0.71742533 atol = 1e-6
opt,sol,data = cs_tssos(pop, x, d, numeq=1, TS="signsymmetry", Gram=true, solution=true, QUIET=true)
@test opt ≈ 0.71742533 atol = 1e-6
...
opt,sol,data = tssos(pop, x, 5, numeq=3, TS=false, GroebnerBasis=false, Gram=true, solution=true, QUIET=true)
@test opt ≈ 1 atol = 1e-6

@polyvar x[1:3]
pop = [1, x[1]^2-2x[1]*x[3]+5, x[1]*x[2]^2+x[2]*x[3]+1, 3x[2]^2-8x[1]*x[3]]
opt,sol,data = tssos(pop, x, 3, numeq=3, TS=false, Gram=true, solution=true, QUIET=true)
@test opt ≈ 1 atol = 1e-6
...
opt,sol,data = tssos([1;h1;h2;h3;h4;h5;h6], x, 3, numeq=6, TS=false, Gram=true, solution=true, QUIET=true, rtol=1e-6)
@test opt ≈ 1 atol = 1e-6
opt,sol,data = tssos([1;x[1]-0.5;h1;h2;h3;h4;h5;h6], x, 2, numeq=6, TS=false, Gram=true, solution=true, QUIET=true, rtol=1e-4)
@test opt ≈ 1 atol = 1e-6
...
opt,sol,data = tssos(F, [G], x, 2, TS="block", QUIET=true, Gram=true, Moment=true)
@test opt ≈ -4 atol = 1e-6
opt,sol,data = tssos(data, QUIET=true, Gram=true)
@test opt ≈ -4 atol = 1e-6

@polyvar x[1:3]
F = [1+x[1]^2 x[1]^2 x[2]^2; x[1]^2 1 x[3]^2; x[2]^2 x[3]^2 1]
G = [2-x[1]^2 1 x[1]+x[2]+x[3]; 1 2-x[2]^2 1; x[1]+x[2]+x[3] 1 2-x[3]^2]
opt,sol,data = tssos(F, [G], x, 2, TS=false, solution=true, Gram=true, QUIET=true)
@test opt ≈ -0.67070657 atol = 1e-6

@polyvar x[1:3]
F = [1+x[1]^2 x[1]^2 x[2]^2; x[1]^2 1 x[3]^2; x[2]^2 x[3]^2 1]
G = [2-x[1]^2 1 x[1]+x[2]+x[3]; 1 2-x[2]^2 1; x[1]+x[2]+x[3] 1 2-x[3]^2]
opt,sol,data = tssos(F, [G], x, 2, TS="block", Gram=true, QUIET=true)
@test opt ≈ -0.6823037 atol = 1e-6
opt,sol,data = tssos(data, QUIET=true)
@test opt ≈ -0.67070657 atol = 1e-6
...
opt,data = tssos_symmetry(pop, x, 2, G, TS=false, QUIET=true)
...
pop = [f, 1 - sum(x.^2)]
G = PermGroup([perm"(1,2,3)", perm"(1,2)"])
opt,data = tssos_symmetry(pop, x, 2, G, numeq=1, TS=false, QUIET=true)
@test opt ≈ -1.3987174 atol = 1e-6
...
pop = [f; 1 .- x.^2]
G = PermGroup([perm"(1,2,3)", perm"(1,2)"])
opt,data = tssos_symmetry(pop, x, 2, G, numeq=3, TS=false, SymmetricConstraint=false, QUIET=true)
...
pop = [f; 1 .- x.^2]
G = PermGroup([perm"(1,2,3)", perm"(1,2)"])
opt,data = tssos_symmetry(pop, x, 2, G, TS=false, SymmetricConstraint=false, QUIET=true)
...
pop = [f, 1 - sum(x.^2)]
G = PermGroup([perm"(1,2,3)", perm"(1,2)"])
opt,data = tssos_symmetry(pop, x, 2, G, SymmetricConstraint=true, QUIET=true)
@test opt ≈ -1.3987174 atol = 1e-6
opt,data = tssos_symmetry(data, QUIET=true)
@test opt ≈ -1.3987174 atol = 1e-6
